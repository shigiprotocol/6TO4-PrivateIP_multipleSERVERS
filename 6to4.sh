...
do
  ip_suffix=`printf "%x\n" $i`
  ip_addr="fdf1:1a4e:ba48:09b${ip_suffix}::1/64"  > /dev/null
  ...
  if [ $? -eq 0 ]; then
    ...
    echo "ip addr add fdf1:1a4e:ba48:09bd::1/64 dev azumi" >> /etc/private.sh
        ip_addr="fdf1:1a4e:ba48:09b${ip_suffix}::1/64"
...
display_notification $'\e[93mAdding cron job for server!\e[0m'
	...
	ping -c 2 fdf1:1a4e:ba48:09b1::2 | sed "s/.*/\x1b[94m&\x1b[0m/" 
	sleep 1
...
# IPv6 address
ip_address="fdf1:1a4e:ba48:09b1::2"
...
do
    ip_suffix=`printf "%x\n" $i`
    ip_addr="fdf1:1a4e:ba48:09b${ip_suffix}::1"
    echo "+---------------------------+"
...
# iran initial IP address
initial_ip="fdf1:1a4e:ba48:09b1::2/64"
ip addr add $initial_ip dev azumi > /dev/null
...
do
  ip_suffix=`printf "%x\n" $i`
  ip_addr="fdf1:1a4e:ba48:09b${ip_suffix}::2/64" > /dev/null
...
# private.sh
    ...
    echo "ip tunnel add azumi mode sit remote $remote_ip local $local_ip ttl 255" >> /etc/private.sh
    echo "ip link set dev azumi up" >> /etc/private.sh
    echo "ip addr add fdf1:1a4e:ba48:09b1::2/64 dev azumi" >> /etc/private.sh
        ip_addr="fdf1:1a4e:ba48:09b${ip_suffix}::2/64"
        echo "ip addr add $ip_addr dev azumi" >> /etc/private.sh
	...
	ping -c 2 fdf1:1a4e:ba48:09b1::1 | sed "s/.*/\x1b[94m&\x1b[0m/" 
	sleep 1
	display_notification $'\e[93mConfiguring keepalive service..\e[0m'
...
# iPv6 address
ip_address="fdf1:1a4e:ba48:09b1::1"
...
do
    ip_suffix=`printf "%x\n" $i`
    ip_addr="fdf1:1a4e:ba48:09b${ip_suffix}::2"
    echo "+---------------------------+"
    echo -e "| \e[92m$ip_addr    \e[0m|"
done
echo "+---------------------------+"
...
do
    ...
    if [ $i -eq 1 ]; then
        device_name="azumi"
        initial_ip="fdf1:1a4e:ba48:09bd::1/64"
        kharej_ip="fdf1:1a4e:ba48:09b1::2/64"
        i_ip="fdf1:1a4e:ba48:09b"
    elif [ $i -eq 2 ]; then
        device_name="azumi2"
...
create_ping_files() {
    ...
    if [ $server_number -eq 1 ]; then
        device_name="azumi"
        initial_ip="fdf1:1a4e:ba48:09bd::1/64"
        kharej_ip="fdf1:1a4e:ba48:09b1::2"
        i_ip="fdf1:1a4e:ba48:09b"
		ping=40
...
do
    ...
    if [ $i -eq 1 ]; then
        device_name="azumi"
        initial_ip="fdf1:1a4e:ba48:09bd::1/64"
        iran_ip="fdf1:1a4e:ba48:09b1::2/64"
        i_ip="fdf1:1a4e:ba48:09b"
    elif [ $i -eq 2 ]; then
        device_name="azumi2"
        initial_ip="fd1d:fc98:b73e:b381::1/64"
